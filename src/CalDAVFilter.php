...
class CalDAVFilter {
    ...
    /**
     * Добавлят фильтр по Datetime полю евента вида
     *  <?xml version="1.0" encoding="utf-8" ?>
        ...
        <C:time-range start="20170922T033146Z" end="20170922T043146Z"/>
        </C:prop-filter>
        </C:comp-filter>
        </C:comp-filter>
        </C:filter>
     ...
     */
    public function mustBeInTimerange ($fieldname,  $start = NULL, $end = NULL) {
        // Are $start and $end in the correct format?
        if ( ( isset($start) and ! preg_match( '#^\d\d\d\d\d\d\d\dT\d\d\d\d\d\dZ$#', $start, $matches ) )
            or ( isset($end) and ! preg_match( '#^\d\d\d\d\d\d\d\dT\d\d\d\d\d\dZ$#', $end, $matches ) ) )
        { trigger_error('$start or $end are in the wrong format. They must have the format yyyymmddThhmmssZ and should be in GMT', E_USER_ERROR); }

        $this->mustIncludes[] = array("mustBeInTimerange", $fieldname,  $start, $end);
    }
    ...
    public function toXML () {
        ...
        $xml .= '<C:comp-filter name="'.$this->resourceType.'">';
        ...
        foreach($this->mustIncludes as $filter) {
            switch($filter[0]) {
                ...
                case "mustIncludeMatchSubstr":
                    ...
                    $xml .= ' collation="i;octet">'.$filter[2].'</C:text-match></C:prop-filter>';
                    break;
                ...
                case "mustBeInTimerange":
                    if($this->resourceType == "VTODO") $xml .= '<C:comp-filter name="VALARM">';
                    $xml .= '<C:prop-filter name="'.$filter[1].'">';
                    $xml .= '<C:time-range';
                    if($filter[2] != NULL) $xml .= ' start="'.$filter[2].'"';
                    if($filter[3] != NULL) $xml .= ' end="'.$filter[3].'"';
                    $xml .= '/>';
                    $xml .= '</C:prop-filter>';
                    if($this->resourceType == "VTODO") $xml .= '</C:comp-filter>';
                    break;
